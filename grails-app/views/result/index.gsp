		...
		<div id="list-result" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="type" title="${message(code: 'result.type.label', default: 'Type')}" />
					
						<g:sortableColumn property="display" title="${message(code: 'result.display.label', default: 'Display')}" />
				...
				<g:each in="${resultInstanceList}" status="i" var="resultInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${resultInstance.id}">${fieldValue(bean: resultInstance, field: "type")}</g:link></td>
					
						<td>${fieldValue(bean: resultInstance, field: "display")}</td>
